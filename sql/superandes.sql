...
SELECT * FROM USER_TAB_PRIVS WHERE TABLE_NAME IN ('INVENTARIOS', 'BODEGAS', 'PRODUCTOS');


SELECT PRODUCTOS.CODBARRAS, PRODUCTOS.NOMBRE, ROUND(SUM(INVENTARIOS.CANTIDAD_OCUPADA) / NULLIF(INVENTARIOS.CAPACIDAD_MAX, 0), 3) as Ocupación, INVENTARIOS.IDBODEGA
FROM INVENTARIOS
INNER  JOIN BODEGAS ON BODEGAS.ID = INVENTARIOS.IDBODEGA    
INNER JOIN PRODUCTOS ON PRODUCTOS.CODBARRAS = INVENTARIOS.CODIGOBARRAS
WHERE PRODUCTOS.CODBARRAS = 51 AND INVENTARIOS.CAPACIDAD_MAX != 0
